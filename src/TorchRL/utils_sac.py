...
from torchrl.envs.transforms import InitTracker, RewardSum, StepCounter, SqueezeTransform
...
def apply_env_transforms(env, max_episode_steps=1000):
    transformed_env = TransformedEnv(
        ...
        Compose(
            ...
            DoubleToFloat(),
            RewardSum(),
            #SqueezeTransform(in_keys=["reward"],out_keys=["reward"],dim=-1)
        ),
    )
...
def make_environment(cfg, logger=None):
    ...
    #    trsf_clone,
    #)
    eval_env = TransformedEnv(env_maker(cfg), trsf_clone)
...
# Custom function to not deal with parallel environments
#def make_environment(cfg, logger):
#    train_env = apply_env_transforms(env_maker(cfg), cfg.env.max_episode_steps)
#    trsf_clone = train_env.transform.clone()
#    eval_env = TransformedEnv(env_maker(cfg), trsf_clone)
#    return train_env, eval_env
...
def log_metrics(logger, metrics, step):
    for metric_name, metric_value in metrics.items():
        if metric_value is None:
            print(f"WARNING: metric '{metric_name}' is None at step {step}")
            continue
        print(metrics)
