...
def main(cfg: DictConfig):  # noqa: F821
    ...
    _env = make_env(cfg.env.env_name, device)
    if cfg.env.wrapped:
        _env = wrap_env(_env, cfg.loss.gamma, evaluate=False) 
    # Create collector
    ...
    loss_module = ClipPPOLoss(
        ...
        clip_value = cfg.loss.clip_range_vf
    )
    ...
    if cfg.logger.backend:
        ...
        logger = get_logger(
            cfg.logger.backend,
            logger_name=f"results/ppo_{cfg.env.env_name}_{cfg.collector.total_frames}/{cfg.hps}/",
    ...
    if cfg.env.wrapped:
        test_env = wrap_env(test_env, cfg.loss.gamma, evaluate=True)
    ...
    for i in range(total_iter):
        ...
        if len(episode_rewards) > 0:
            ...
            metrics_to_log.update(
                {
                    "rollout/ep_rew_mean": episode_rewards.mean().item(),
                    "rollout/ep_len_mean": episode_length.sum().item()
                    / len(episode_length),
                }
            )
        ...
        with torch.no_grad(), set_exploration_type(
            ...
            if ((i - 1) * frames_in_batch) // cfg_logger_test_interval < (
                ...
                metrics_to_log.update(
                    {
                        "eval/mean_reward": test_rewards.mean(),
                    }
                )
                actor.train()
...
if __name__ == "__main__":
    ...
    cfg = OmegaConf.load("config_ppo.yaml")  
    
    # Manually set the parameters of the hps config file over to the cfg 
    cfg.collector.total_frames = config['steps'] # denk ik
    cfg.collector.frames_per_batch = config['train']['n_steps']
    cfg.optim.lr = config['train']['learning_rate']
    cfg.loss.mini_batch_size = config['train']['batch_size']
    cfg.loss.gamma = config['train']['gamma']
    cfg.loss.gae_lambda = config['train']['gae_lambda']
    cfg.loss.clip_epsilon = config['train']['clip_range']
    cfg.loss.clip_range_vf = config['train']['clip_range_vf']  # Was constant, changed to variable
    cfg.loss.entropy_coef = config['train']['ent_coef']
    cfg.loss.critic_coef = config['train']['vf_coef']
    # Max grad norm ignore
    cfg.optim.anneal_lr = config['train']['anneal_lr']
    cfg.env.env_name = config['env']
    cfg.env.wrapped = config['environment']['wrapped']
